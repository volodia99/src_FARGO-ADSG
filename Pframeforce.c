...
void InitDustVelocities (DVr, DVt, DRho)
...
void InitDustStoppingTime (Rho, Vr, Vt, DVr, DVt)
     PolarGrid *Rho, *Vr, *Vt, *DVr, *DVt;
{
  int i, j, l, nr, ns;
  real Cdrag, Re, k_D, f_D, Kn, lambda, densg_cgs, Mach;
  real dust_internal_density, dust_radius;
  real *dens, *vr, *dvr, *vt, *dvt, *cs, *ts;
  ...
  if(DustFeelDisk){
	...
	Mach = sqrt( (dvr[l]-vr[l])*(dvr[l]-vr[l]) + (dvt[l]-vt[l])*(dvt[l]-vt[l]) ) / cs[l];
	...
	ts[l] =  0.5*M_PI*Cdrag*dust_internal_density*dust_radius/dens[l];
      }
    }
  }
}
