...
{
  Pair acceleration;
  ComputeForce (force, Rho, x, y, rsmoothing, mass, psys, index);
  if (ExcludeHill) {
...
{
  int i,j,l,ns,nr;
  extern boolean DontApplySubKeplerian, RadiativeDiffusion;
...
#pragma omp parallel for private(l)
	...
	if (RadiativeDiffusion) {
	  /*
	  T1 = energy[l]/rho[l];
	  T2 = energy[l+ns]/rho[l+ns];
	  R0 = Rmed[i-1];
	  R1 = Rmed[i];
	  R2 = Rmed[i+1];
	  Rsup0 = Rsup[i-1];
	  Rsup1 = Rsup[i];
	  Ksup0 = ((R1-Rsup0)*kcoeff[l-ns] + (Rsup0-R0)*kcoeff[l])/(R1-R0);
	  Ksup1 = ((R2-Rsup1)*kcoeff[l] + (Rsup1-R1)*kcoeff[l+ns])/(R2-R1);
	  T0 = T1 - ((Rsup1*Ksup1)/(Rsup0*Ksup0))*(R1-R0)*(T2-T1)/(R2-R1);
	  energy[l-ns] = T0*rho[l-ns];
	  printf ("T0 = %lg, T1 = %lg, T2 = %lg\n",T0,T1,T2);
	  printf ("e0 = %lg, e1 = %lg, e2 = %lg\n",energy[l-ns],energy[l],energy[l+ns]);
	  //energy[l-ns] = rho[l-ns] * ( energy[l]/rho[l] + ((energy[l]/rho[l]) - (energy[l+ns]/rho[l+ns]))/(Rmed[i]-Rmed[i+1])*(Rmed[i-1]-Rmed[i])*(Rmed[i+1]/Rmed[i])*(kcoeff[l+ns]/kcoeff[l]) );
	  //energy[l-ns] = energy[l] * rho[l-ns]/rho[l];   // zero gradient in gas temperature
	  */
	  energy[l-ns] = energy[l] * Rmed[i]/Rmed[i-1];   // e or T in 1/R (uniform sigma)
	  //energy[l-ns] = energy[l] * rho[l-ns]/rho[l];
	  //energy[l-ns] = (rho[l-ns]/rho[l]) * energy[l] * pow(Rmed[i-1]/Rmed[i],-0.2*(3.0+2.0*SIGMASLOPE));   // zero gradient in RKdT/dR assuming kappa ~ T^2
	  //energy[l-ns] = energy[l];//EnergyMed[0];//energy[l];
	} else {
	  energy[l-ns] = energy[l]; // zero gradient for thermal energy
	}
...
#pragma omp parallel for private(l)
	...
	rho[l] = rho[l-ns];	// zero gradient for surface density
      if (RadiativeDiffusion) {
	/*
	Tnm3 = energy[l-2*ns]/rho[l-2*ns];
	Tnm2 = energy[l-ns]/rho[l-ns];
	Rnm3 = Rmed[i-2];
	Rnm2 = Rmed[i-1];
	Rnm1 = Rmed[i];
	Rsupnm3 = Rsup[i-2];
	Rsupnm2 = Rsup[i-1];
	Ksupnm3 = ((Rnm2-Rsupnm3)*kcoeff[l-2*ns] + (Rsupnm3-Rnm3)*kcoeff[l-ns])/(Rnm2-Rnm3);
	Ksupnm2 = ((Rnm1-Rsupnm2)*kcoeff[l-ns] + (Rsupnm2-Rnm2)*kcoeff[l])/(Rnm1-Rnm2);
	Tnm1 = Tnm2 - ((Rsupnm3*Ksupnm3)/(Rsupnm2*Ksupnm2))*(Rnm2-Rnm1)*(Tnm3-Tnm2)/(Rnm3-Rnm2);
	energy[l] = Tnm1*rho[l];
	//energy[l] = rho[l] * ( energy[l-ns]/rho[l-ns] + ((energy[l-ns]/rho[l-ns]) - (energy[l-2*ns]/rho[l-2*ns]))/(Rmed[i-1]-Rmed[i-2])*(Rmed[i]-Rmed[i-1])*(Rmed[i-2]/Rmed[i-1])*(kcoeff[l-2*ns]/kcoeff[l-ns]) );
	//energy[l] = energy[l-ns] * rho[l] / rho[l-ns];  // zero gradient in temperature
	*/
	energy[l] = energy[l-ns] * Rmed[i-1]/Rmed[i];  // e or T in 1/R (uniform sigma)
	//energy[l] = energy[l-ns] * rho[l] / rho[l-ns];
	//energy[l] = (rho[l]/rho[l-ns]) * energy[l-ns] * pow(Rmed[i]/Rmed[i-1],-0.2*(3.0+2.0*SIGMASLOPE));   // zero gradient in RKdT/dR assuming kappa ~ T^2
	//energy[l] = energy[l-ns];//EnergyMed[i];//energy[l-ns];
      } else {
...
#pragma omp parallel for private(l,jp,lp,vr_med)
	...
	if (AccBoundary == YES) {
	  //printf ("in bef: vr[%d] = %lg\n",l,vr[l]);
	  vr[l] += (2.*vr_med-vr[l+ns]);
	  //printf ("in aft: vr[%d] = %lg\n",l,vr[l]);
	}
	else
	  vr[l] = 2.*vr_med-vr[l+ns];
      }
...
#pragma omp parallel for private(l,jp,lp,vr_med)
	...
	vr_med = cs[l]*(rho[l-ns]-densnrm2)/densnrm2;
      if (AccBoundary == YES) {
	//printf ("out bef: vr[%d] = %lg\n",l,vr[l]);
	vr[l] += (2.*vr_med-vr[l-ns]);
	//printf ("out aft: vr[%d] = %lg\n",l,vr[l]);
      }
      else
	vr[l] = 2.*vr_med-vr[l-ns];
    }
     ...
     PolarGrid *Rho, *Vrad;
{
  ...
  extern boolean DiskWind, BetaConstant;
  nr = Rho->Nrad;
  ns = Rho->Nsec;
  rho= Rho->Field;
  vr = Vrad->Field;
  if (CPU_Rank == CPU_Highest) {
    ...
    if ((DiskWind) && (BetaConstant)) {
      penul_vr = -2.0*0.4*sqrt(2.0/M_PI)*ASPECTRATIO*INVBETAPLASMA*pow((Rinf[nr-1]/1.0),FLARINGINDEX-0.5);
      // penul_vr = 0.0;
    }
    for (j = 0; j < ns; j++) {
      l = j+i*ns;
  ...
  if (CPU_Rank == 0) {
    ...
    for (j = 0; j < ns; j++) {
      l = j+i*ns;
      average_rho += rho[l-ns];
    }
    average_rho /= (real)ns;
    average_rho = SigmaMed[i-1]-average_rho;
    for (j = 0; j < ns; j++) {
      l = j+i*ns;
      rho[l-ns] += average_rho;
    }
    penul_vr = IMPOSEDDISKDRIFT*pow((Rinf[i]/1.0),SIGMASLOPE-1.0);
    if ((DiskWind) && (BetaConstant)) {
      penul_vr = -2.0*0.4*sqrt(2.0/M_PI)*ASPECTRATIO*INVBETAPLASMA*pow((Rinf[i]/1.0),FLARINGINDEX-0.5);
      // penul_vr = 0.0;
    }
    for (j = 0; j < ns; j++) {
      l = j+i*ns;
    ...
    penul_vr = IMPOSEDDISKDRIFT*pow((Rinf[i-1]/1.0),SIGMASLOPE-1.0);
    if ((DiskWind) && (BetaConstant)) {
      penul_vr = -2.0*0.4*sqrt(2.0/M_PI)*ASPECTRATIO*INVBETAPLASMA*pow((Rinf[i-1]/1.0),FLARINGINDEX-0.5);
      // penul_vr = 0.0;
    }
    for (j = 0; j < ns; j++) {
      l = j+i*ns;
