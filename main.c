     ...
     char *argv[];
{
  ...
  if ( SelfGravity ) {
    ...
    compute_selfgravity (gas_density, gas_v_rad, gas_v_theta, dust_v_rad, dust_v_theta, foostep, updatevelocities);
  ...
  for (i = begin_i; i <= NTOT; i++) {
    ...
    if ( (!SoftWriting) && (InnerOutputCounter == 1)) {
      ...
      SolveOrbits (sys);
      UpdateLog (force, sys, gas_density, gas_energy, TimeStep, PhysicalTime);
      if (Stockholm == YES)
    ...
    if (NINTERM * (TimeStep = (i / NINTERM)) == i) {
      ...
      TimeToWrite = YES;
      if ( (NBPART != 0) && Write_DustDensity)
      	ComputeDustPcDensity (dustsys, dust_pc_density);
    ...
    AlgoGas (force, gas_density, gas_v_rad, gas_v_theta, gas_energy, gas_label, dust_density, dust_v_rad, dust_v_theta, sys, dustsys);
