...
extern boolean  Write_Potential, Write_Test, Write_DustDensity, Write_StoppingTime;
     ...
     int timestep;
{
  ...
  for (k = 0; k < sys->nb; k++) {
    ...
    if ( fabs((bufmass-sys->mass[k])/bufmass) > 1e-4 ) {
      ...
      sys->vy[k] = bufvy;
      sys->mass[k] = bufmass;
      masterprint ("At restart mass of planet %d is %lg\n", k, sys->mass[k]);
    }
  }
}
     ...
     DustSystem  *dustsys;
{
  ...
  if (IsDisk == YES) {
    ...
    if ((Write_StoppingTime == YES) && (DustFluid) ) WriteDiskPolar (TStop, index);
    MPI_Barrier (MPI_COMM_WORLD);
