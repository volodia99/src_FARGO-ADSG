...
extern boolean FastTransport, IsDisk, AddFloors, DustFluid, DampToViscous, ShortFrictionTimeApproximation;
extern boolean AddMass, DontApplySubKeplerian, DiscEvaporation, PhotoEvaporation;
     ...
     PlanetarySystem *sys;
{
  ...
  TStop	       = CreatePolarGrid(NRAD, NSEC, "dustTStop"); /* dust stokes number */
  ...
  if (DustFluid)
    InitDustStoppingTime (Rho, Vr, Vt, DVr, DVt);
...
void AlgoGas (force, Rho, Vrad, Vtheta, Energy, Label, DRho, DVrad, DVtheta, sys, dsys)
     Force *force;
     PolarGrid *Rho, *Vrad, *Vtheta, *Energy, *Label;
     PolarGrid *DRho, *DVrad, *DVtheta;
     PlanetarySystem *sys;
     DustSystem *dsys;
{
  ...
  while (dtemp < 0.999999999*DT) {
    ...
    if (NBPART != 0) {
      ...
      UpdateDustVelocities(dsys, sys, Vrad, Vtheta, Rho, dt);  // update particles velocities over dt
      //SemiUpdateDustPositions(dsys, sys, Rho, dt);  // update particles positions over dt/2
    }
     ...
     real dt;
{
  ...
  real *dvradint, *dvthetaint, *dPress, *ts;
  ...
  if (DustFluid) {
	...
	ts[l] =  0.5*M_PI*Cdrag*dust_internal_density*dust_radius/rho[l];
      }
    }
  }
...
void SubStep3 (Rho, Vtheta, dt)
     ...
     real dt;
{
  extern boolean TempPresc, BetaCooling, DustFeedback, StellarIrradiation, ImposedStellarLuminosity;
  int i, j, l, nr, ns;
     ...
     real deltaT;
{
  ...
  real *dustvr, *dustvt, *ts, csd, ddvr, ddvt;
  real *soundspeed, *dsoundspeed, *temperature, *dens, *opacity;
  extern boolean DustFeedback, DustFluid, DustFeelDisk, Evanescent, DampToViscous;
  if (EnergyEquation && RadiativeDiffusion && !ImplicitRadiativeDiffusion)
     ComputeOpacities (Rho, Energy);
  opacity = Opacity->Field;
  ...
  for (i = One_or_active; i < Max_or_active; i++) {
	...
	if(ShortFrictionTimeApproximation == NO) {
	  ...
	  if(DustFeelDisk==NO)
	    ...
	    invdt10 = 1./(ts[l]+1.e-6)*sqrt(G*1.0/Rmed[i])/Rmed[i];
	} else {
	  invdt7=1.e-10;
	  invdt8=1.e-10;
	  invdt9=1.e-10;
  ...
  if (DustFluid && (ShortFrictionTimeApproximation == NO)) {
    for (i = Zero_or_active; i < MaxMO_or_active; i++) {
  ...
  if (debug == YES) {
    ...
    if (DustFluid) {
      ...
      printf ("Dust Radial aerodrag limit     : %g (dvr=%g, vr=%g, ts=%g)\n", itdbg10, dustvr[jdeb+ideb*ns], vr[jdeb+ideb*ns], ts[jdeb+ideb*ns]);
     ...
     PolarGrid *Rho, *DRho, *Vrad, *Vtheta,*DVrad,*DVtheta;
{
  ...
  real *rho, *cs, *drho, *vrad, *vtheta, *dvrad, *dvtheta, *ts, *diag1, *diag2, *diag3;
  ...
  for (i = 1; i < nr; i++){
    for (j = 0; j < ns; j++){
      ...
      dvrad[l]=vrad[l]+1./sqrt(G*1.0/Rmed[i])*Rmed[i]*ts[l]*(gradp+1e-15);
      //	diag1[l]=rho[l];
  ...
  for (i = 0; i < nr; i++) {
    ...
    for (j = 0; j < ns; j++) {
      ...
      ljp = l+1;
      if (j == ns-1) ljp = i*ns;
      gradp = cs[l]*cs[l]*(rho[l]-rho[ljm])*2.0/(rho[l]+rho[ljm])*invdxtheta;
      dvtheta[l]=vtheta[l]+1./sqrt(G*1.0/Rmed[i])*Rmed[i]*ts[l]*(gradp+1e-15);
      //        diag3[l]=dvtheta[l];
    }
  }
}
...
{
  int i, j, l, lim, lip, ljm, ljp, nr, ns;
  real *drho, *rho, *dvrad, *dvtheta, *fdiffrp, *fdifftp, *diag3, *ts;
  ...
  for (i = 0; i < nr-1; i++){
    ...
    for (j = 0; j < ns; j++){
      ...
      ljp = l+1;
      if (j == ns-1) ljp = i*ns;
      Dturb = viscosityp*(1.0+4.0*ts[lip]*ts[lip])*pow((1.0+ts[lip]*ts[lip]),-2.0);
      fdiffrp[l]=-Dturb*(rho[lip]+rho[l])/2.*(drho[lip]/rho[lip]-drho[l]/rho[l])*InvDiffRmed[i+1];
      Dturb = viscosity*(1.0+4.0*ts[l]*ts[l])*pow((1.0+ts[l]*ts[l]),-2.0);
